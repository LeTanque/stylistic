...
function LocalNTP() {
    ...
    function areRealboxMatchesVisible() {
        return getElem(IDS.REALBOX_INPUT_WRAPPER).classList.contains(CLASSES.SHOW_MATCHES);
    }

    /** @param {!AutocompleteResult} result */
    function autocompleteResultChanged(result) {
        if (lastQueriedInput === undefined || result.input !== lastQueriedInput.trimLeft()) {
    ...
    function clearAutocompleteMatches() {
        ...
        window.chrome.embeddedSearch.searchBox.stopAutocomplete(/*clearResult=*/ true);
    ...
    function createIframes() {
        ...
        args.push('title=' + encodeURIComponent(configData.translatedStrings.mostVisitedTitle));
        args.push(
            'removeTooltip=' +
        ...
        if (configData.isGooglePage) {
            ...
            args.push('addLink=' + encodeURIComponent(configData.translatedStrings.addLinkTitle));
            args.push(
                'addLinkTooltip=' + encodeURIComponent(configData.translatedStrings.addLinkTooltip)
            );
            args.push(
                'editLinkTooltip=' +
        ...
        getElem(IDS.TILES).appendChild(iframe);
        ...
        if (configData.isGooglePage) {
            ...
            clArgs.push(
                'editTitle=' + encodeURIComponent(configData.translatedStrings.editLinkTitle)
            );
            clArgs.push('nameField=' + encodeURIComponent(configData.translatedStrings.nameField));
            clArgs.push('urlField=' + encodeURIComponent(configData.translatedStrings.urlField));
            clArgs.push(
                'linkRemove=' + encodeURIComponent(configData.translatedStrings.linkRemove)
            );
            clArgs.push(
                'linkCancel=' + encodeURIComponent(configData.translatedStrings.linkCancel)
            );
            clArgs.push('linkDone=' + encodeURIComponent(configData.translatedStrings.linkDone));
            clArgs.push(
                'invalidUrl=' + encodeURIComponent(configData.translatedStrings.invalidUrl)
            );
    ...
    function customLinksEnabled() {
        return configData.isGooglePage && !chrome.embeddedSearch.newTabPage.isUsingMostVisited;
    }
    ...
    function floatDownNotification(notification, notificationContainer, showPromo) {
        ...
        notificationContainer.addEventListener(
            ...
            event => {
                ...
                getElem(IDS.UNDO_LINK).blur();
                getElem(IDS.RESTORE_ALL_LINK).blur();
                if (notification.classList.contains(CLASSES.HAS_LINK)) {
                    notification.classList.remove(CLASSES.HAS_LINK);
                    getElem(IDS.ERROR_NOTIFICATION_LINK).blur();
                }
                // Hide the notification
    ...
    function getUseDarkChips(info) {
        return window.matchMedia('(prefers-color-scheme: dark)').matches && !info.imageUrl;
    }
    ...
    function handlePostMessage(event) {
        ...
        if (cmd === 'loaded') {
            ...
            if (configData.isGooglePage) {
                ...
                // hidden.
                if (getElem(IDS.SUGGESTIONS)) {
                    getElem(IDS.SUGGESTIONS).style.visibility = 'visible';
                }
                if (getElem(IDS.PROMO)) {
                    showPromoIfNotOverlappingAndTrackResizes();
                }
                if (customLinksEnabled()) {
                    $(customize.IDS.CUSTOM_LINKS_RESTORE_DEFAULT).classList.toggle(
                        customize.CLASSES.OPTION_DISABLED,
                        !args.showRestoreDefault
                    );
                    $(customize.IDS.CUSTOM_LINKS_RESTORE_DEFAULT).tabIndex = args.showRestoreDefault
                        ? 0
                        : -1;
                }
                getElem(IDS.OGB).classList.add(CLASSES.SHOW_ELEMENT);
            }
        } else if (cmd === 'tileBlacklisted') {
            if (customLinksEnabled()) {
                showNotification(configData.translatedStrings.linkRemovedMsg);
            } else {
                showNotification(configData.translatedStrings.thumbnailRemovedNotification);
            }
            ...
            getElem(IDS.CUSTOM_LINKS_EDIT_IFRAME).contentWindow.postMessage(
                { cmd: 'linkData', rid: args.rid },
                '*'
            );
            // Small delay to allow the dialog to finish setting up before displaying.
            window.setTimeout(function () {
                getElem(IDS.CUSTOM_LINKS_EDIT_IFRAME_DIALOG).showModal();
            }, 10);
        } else if (cmd === 'closeDialog') {
            getElem(IDS.CUSTOM_LINKS_EDIT_IFRAME_DIALOG).close();
            ...
            const iframe = getElem(IDS.TILES_IFRAME);
            if (iframe) {
    ...
    function init() {
        ...
        getElem(IDS.NOTIFICATION_MESSAGE).textContent =
            configData.translatedStrings.thumbnailRemovedNotification;

        const undoLink = getElem(IDS.UNDO_LINK);
        undoLink.addEventListener('click', onUndo);
        registerKeyHandler(undoLink, ['Enter', ' '], onUndo);
        undoLink.textContent = configData.translatedStrings.undoThumbnailRemove;

        const restoreAllLink = getElem(IDS.RESTORE_ALL_LINK);
        restoreAllLink.addEventListener('click', onRestoreAll);
        registerKeyHandler(restoreAllLink, ['Enter', ' '], onRestoreAll);

        getElem(IDS.ATTRIBUTION_TEXT).textContent = configData.translatedStrings.attributionIntro;
        ...
        if (configData.isGooglePage) {
            ...
            if (configData.realboxEnabled) {
                ...
                const realboxEl = getElem(IDS.REALBOX);
                ...
                const realboxWrapper = getElem(IDS.REALBOX_INPUT_WRAPPER);
                ...
                searchboxApiHandle.autocompletematchimageavailable = autocompleteMatchImageAvailable;

                if (!iframesAndVoiceSearchDisabledForTesting) {
                    speech.init(
                        configData.googleBaseUrl,
                        configData.translatedStrings,
                        getElem(IDS.REALBOX_MICROPHONE),
                        searchboxApiHandle
                    );
                }

                utils.disableOutlineOnMouseClick(getElem(IDS.REALBOX_MICROPHONE));
            } else {
                ...
                getElem(IDS.FAKEBOX_TEXT).textContent = configData.translatedStrings.searchboxPlaceholder;

                if (!iframesAndVoiceSearchDisabledForTesting) {
                    speech.init(
                        configData.googleBaseUrl,
                        configData.translatedStrings,
                        getElem(IDS.FAKEBOX_MICROPHONE),
                        searchboxApiHandle
                    );
                }
                ...
                const inputbox = getElem(IDS.FAKEBOX_INPUT);
                ...
                utils.disableOutlineOnMouseClick(getElem(IDS.FAKEBOX_MICROPHONE));
        ...
        if (searchboxApiHandle.rtl) {
            getElem(IDS.NOTIFICATION).dir = 'rtl';
        }
    ...
    function injectPromo(promo) {
        ...
        getElem(IDS.NTP_CONTENTS).appendChild(promoContainer);
    ...
    function injectSearchSuggestions(suggestions) {
        ...
        getElem(IDS.USER_CONTENT).insertAdjacentElement('afterbegin', suggestionsContainer);
    ...
    function isFakeboxClick(event) {
        return (
            getElem(IDS.FAKEBOX).contains(/** @type HTMLElement */ (event.target)) &&
            !getElem(IDS.FAKEBOX_MICROPHONE).contains(/** @type HTMLElement */ (event.target))
        );
    }
    ...
    function isFakeboxFocused() {
        return (
            document.body.classList.contains(CLASSES.FAKEBOX_FOCUS) ||
    ...
    function isPromoOverlapping() {
        ...
        const promoRect = getElem(IDS.PROMO).querySelector('div').getBoundingClientRect();
        ...
        if (window.chrome.embeddedSearch.searchBox.rtl) {
            ...
            if (attributionRect.width > 0 && promoRect.left - MARGIN <= attributionRect.right) {
                return true;
            }
            ...
            if (
                customAttributionsRect.width > 0 &&
            ...
            const attributionEl = getElem(IDS.ATTRIBUTION);
            const attributionRect = attributionEl.getBoundingClientRect();
            if (attributionRect.width > 0) {
                const attributionOnLeft = attributionEl.classList.contains(
    ...
    function navigateToMatch(match, e) {
        ...
        window.chrome.embeddedSearch.searchBox.openAutocompleteMatch(
            ...
            areRealboxMatchesVisible(),
            Date.now() - lastRealboxFocusTime,
            e.button || 0,
            e.altKey,
            e.ctrlKey,
            e.metaKey,
    ...
    function onAddCustomLinkDone(success) {
        if (success) {
            ...
            showErrorNotification(configData.translatedStrings.linkCantCreate, null, null);
        }
    ...
    function onDeleteCustomLinkDone(success) {
        if (success) {
            ...
            showErrorNotification(configData.translatedStrings.linkCantRemove, null, null);
        }
    }
    ...
    function onRealboxCutCopy(e) {
        ...
        if (
            !realboxEl.value ||
            realboxEl.selectionStart !== 0 ||
            realboxEl.selectionEnd !== realboxEl.value.length ||
            !autocompleteResult ||
            autocompleteResult.matches.length === 0
        ) {
    ...
    function onRealboxInput() {
        const realboxValue = getElem(IDS.REALBOX).value;
    ...
    function onRealboxKeyup(e) {
        if (e.key === 'Tab' && !getElem(IDS.REALBOX).value) {
            queryAutocomplete('');
        }
    }
    ...
    function onRealboxMatchesFocusIn(e) {
        ...
        updateRealboxOutput({
            ...
            inline: '',
            text: selectedMatch.fillIntoEdit
        });
    }
    ...
    function onRealboxWrapperFocusOut(e) {
        ...
        if (!realboxWrapper.contains(relatedTarget)) {
            ...
            // Enter.
            window.chrome.embeddedSearch.searchBox.stopAutocomplete(/*clearResult=*/ false);
        }
    }
    ...
    function onRealboxWrapperKeydown(e) {
        ...
        if (e.target === realboxEl && lastOutput.inline) {
            ...
            if (
                realboxSelected === lastOutput.inline &&
        ...
        const realboxMatchesEl = getElem(IDS.REALBOX_MATCHES);
    ...
    function onUpdateCustomLinkDone(success) {
        if (success) {
            ...
            showErrorNotification(configData.translatedStrings.linkCantEdit, null, null);
        }
    }
    ...
    function queryAutocomplete(input) {
        ...
        const preventInlineAutocomplete =
            isDeletingInput || pastedInRealbox || getElem(IDS.REALBOX).selectionStart !== input.length; // Caret not at the end.
        window.chrome.embeddedSearch.searchBox.queryAutocomplete(input, preventInlineAutocomplete);
    }
    ...
    function reloadTiles() {
        ...
        const cmds = [];
        const maxNumTiles = customLinksEnabled()
        ...
        getElem(IDS.MOST_VISITED).hidden = !chrome.embeddedSearch.newTabPage.areShortcutsVisible;
    ...
    function renderAutocompleteMatches(matches, suggestionGroupsMap) {
        ...
        function createSuggestionGroupEl(suggestionGroupId) {
            ...
            function updateToggleButtonA11y(toggleButtonEl, groupIsHidden) {
                ...
                toggleButtonEl.ariaLabel = utils.substituteString(
                    groupIsHidden
                        ? configData.translatedStrings.showSection
            ...
            if (configData.suggestionTransparencyEnabled) {
                const toggle = createActionButton(() => {
                    ...
                    updateToggleButtonA11y(toggle, groupEl.classList.contains(CLASSES.COLLAPSED));
        ...
        for (let i = 0; i < matches.length; ++i) {
            ...
            if (hasImage) {
                ...
                if (faviconOrImageUrlToDataUrlCache[match.destinationUrl]) {
                    setBackgroundImageByUrl(
                        iconEl,
                        faviconOrImageUrlToDataUrlCache[match.destinationUrl]
                    );
            ...
            const contentsEl = renderMatchClassifications(match.contents, match.contentsClass);
            let descriptionEl;
            let separatorEl;
            let separatorText = '';

            if (match.description) {
                descriptionEl = renderMatchClassifications(
                    match.description,
            ...
            if (match.suggestionGroupId && match.suggestionGroupId !== NO_SUGGESTION_GROUP_ID) {
        ...
        getElem(IDS.REALBOX_MATCHES).remove();
        ...
        return classifications
            .map((classification, i) => {
                ...
                return classes.length
                    ? spanWithClasses(classifiedText, classes)
    ...
    function renderTheme() {
        ...
        const useWhiteLogo = theme.alternateLogo || (theme.usingDefaultTheme && isDarkModeEnabled);
        document.body.classList.toggle(CLASSES.ALTERNATE_LOGO, useWhiteLogo);

        if (theme.logoColor) {
            document.body.style.setProperty('--logo-color', convertToRGBAColor(theme.logoColor));
        }
        ...
        document.body.classList.toggle(
            CLASSES.DONT_SHOW_DOODLE,
            !theme.usingDefaultTheme || !!theme.imageUrl
        );
        ...
        if (!theme.customBackgroundConfigured) {
            document.body.style.background = [
                convertToRGBAColor(theme.backgroundColorRgba),
                theme.imageUrl,
                theme.imageTiling,
                theme.imageHorizontalAlignment,
                theme.imageVerticalAlignment
            ]
                .join(' ')
                .trim();

            getElem(IDS.CUSTOM_BG).style.opacity = '0';
            getElem(IDS.CUSTOM_BG).style.backgroundImage = '';
            customize.clearAttribution();
        } else {
            // Do anything only if the custom background changed.
            const imageUrl = assert(theme.imageUrl);
            if (!getElem(IDS.CUSTOM_BG).style.backgroundImage.includes(imageUrl)) {
                ...
                if (!configData.richerPicker && imageWithOverlay.includes(localImageFileName)) {
                ...
                getElem(IDS.CUSTOM_BG).style.backgroundImage = imageWithOverlay;
                const image = new Image();
                image.onload = function () {
                    getElem(IDS.CUSTOM_BG).style.opacity = '1';
                };
                image.src = imageUrl;
        ...
        $(customize.IDS.RESTORE_DEFAULT).classList.toggle(
            customize.CLASSES.OPTION_DISABLED,
            !theme.customBackgroundConfigured
        );
        $(customize.IDS.RESTORE_DEFAULT).tabIndex = theme.customBackgroundConfigured ? 0 : -1;

        $(customize.IDS.EDIT_BG).classList.toggle(
            CLASSES.ENTRY_POINT_ENHANCED,
            !theme.customBackgroundConfigured
        );
        ...
        if (configData.realboxMatchOmniboxTheme) {
            ...
            removeMatchHovered[3] = 0.16 * 255;

            const removeMatchSelectedHovered = assert(theme.searchBox.iconSelected).slice();
            removeMatchSelectedHovered[3] = 0.16 * 255;

            const removeMatchFocused = theme.searchBox.iconSelected.slice();
            removeMatchFocused[3] = 0.32 * 255;
            ...
            setCssVar('search-box-results-bg-hovered', theme.searchBox.resultsBgHovered);
            setCssVar('search-box-results-bg-selected', theme.searchBox.resultsBgSelected);
            setCssVar('search-box-results-dim', theme.searchBox.resultsDim);
            setCssVar('search-box-results-dim-selected', theme.searchBox.resultsDimSelected);
            setCssVar('search-box-results-text', theme.searchBox.resultsText);
            setCssVar('search-box-results-text-selected', theme.searchBox.resultsTextSelected);
            setCssVar('search-box-results-url', theme.searchBox.resultsUrl);
            setCssVar('search-box-results-url-selected', theme.searchBox.resultsUrlSelected);
    ...
    function setAttributionVisibility(show) {
        getElem(IDS.ATTRIBUTION).style.display = show ? '' : 'none';
    }
    ...
    function setRealboxIcon(match) {
        const realboxIcon = getElem(IDS.REALBOX_ICON);
        if (match && !match.isSearchType) {
            ...
            // match icon.
            /** @suppress {missingProperties} */
            realboxIcon.dataset.icon = match ? match.iconUrl : realboxIcon.dataset.defaultIcon;
            setWebkitMaskImageByUrl(realboxIcon, realboxIcon.dataset.icon);
        }
    }
    ...
    function setRealboxMatchesVisible(visible) {
        getElem(IDS.REALBOX_INPUT_WRAPPER).classList.toggle(CLASSES.SHOW_MATCHES, visible);
    }
    ...
    function showErrorNotification(msg, linkName, linkOnClick) {
        const notification = getElem(IDS.ERROR_NOTIFICATION);
        getElem(IDS.ERROR_NOTIFICATION_MSG).textContent = msg;
        if (linkName && linkOnClick) {
            const notificationLink = getElem(IDS.ERROR_NOTIFICATION_LINK);
        ...
        floatUpNotification(notification, getElem(IDS.ERROR_NOTIFICATION_CONTAINER));
    }
    ...
    function showNotification(msg) {
        getElem(IDS.NOTIFICATION_MESSAGE).textContent = msg;
        getElem(IDS.RESTORE_ALL_LINK).textContent = customLinksEnabled()
            ? configData.translatedStrings.restoreDefaultLinks
            : configData.translatedStrings.restoreThumbnailsShort;
        floatUpNotification(getElem(IDS.NOTIFICATION), getElem(IDS.NOTIFICATION_CONTAINER));
        getElem(IDS.UNDO_LINK).focus();
    }

    function showPromoIfNotOverlapping() {
        getElem(IDS.PROMO).style.visibility = isPromoOverlapping() ? 'hidden' : 'visible';
    }
    ...
    function updateRealboxOutput(update) {
        ...
        const realboxEl = getElem(IDS.REALBOX);
        const newOutput = /** @type {!RealboxOutput} */ (Object.assign({}, lastOutput, update));
        ...
        if (newAll.trim() && needsSelectionUpdate) {
            realboxEl.selectionStart = preserveSelection
                ? oldSelectionStart
                : newOutput.text.length;
            ...
            realboxEl.selectionEnd = preserveSelection ? oldSelectionStart : newAll.length;
        }
    ...
    function updateThemeAttribution(url, themeBackgroundAlignment) {
        ...
        const attribution = getElem(IDS.ATTRIBUTION);
